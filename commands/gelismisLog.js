...
module.exports = (client) => {
    ...
    client.on('roleCreate', async (role) => {
        ...
        const embed = new EmbedBuilder()
            .setTitle('<:rol:1505201454615629845> Yeni Rol <a:tik:1505164671081123840> Oluşturuldu!')
    ...
    client.on('guildMemberUpdate', async (oldMember, newMember) => {
        ...
        if (!oldTimeout && newTimeout) {
            const embed = new EmbedBuilder()
                ...
                .setDescription(`<:uzaybot_kullanicilar:1505146190973505567> **Susturulan:** ${newMember.user}\n⏳ **Ceza Bitiş:** <t:${Math.round(newTimeout / 1000)}:R>`)
                .setColor('#f1c40f').setTimestamp();
            logChan.send({ embeds: [embed] }).catch(() => {});
        } else if (oldTimeout && !newTimeout) {
            const embed = new EmbedBuilder()
                .setTitle('<:Ses:1505164439505342484> Susturulma Kaldırıldı!')
                .setDescription(`<:uzaybot_kullanicilar:1505146190973505567> **Üye:** ${newMember.user} ceza süresi bitti veya el ile kaldırıldı kanka.`)
                .setColor('#2ecc71').setTimestamp();
            logChan.send({ embeds: [embed] }).catch(() => {});
        }
    });
    ...
    client.on('voiceStateUpdate', async (oldState, newState) => {
        ...
        if (!oldState.channelId && newState.channelId) {
            ...
            const embed = new EmbedBuilder()
                .setTitle('<a:join_join:1505202309343215717> Ses Kanalına Giriş Yapıldı!')
                .setDescription(`<:uzaybot_kullanicilar:1505146190973505567> ${üye} -> ${newState.channel} odasına giriş yaptı.`)
                .setColor('#1abc9c').setTimestamp();
            logChan.send({ embeds: [embed] }).catch(() => {});
        } else if (oldState.channelId && !newState.channelId) {
            // Sesten tamamen çıktı kanka
            const embed = new EmbedBuilder()
                .setTitle('<a:Leave_Leave:1505202549706195004> Ses Kanalından Çıkış Yapıldı!')
            ...
            const embed = new EmbedBuilder()
                ...
                .setDescription(`<:uzaybot_kullanicilar:1505146190973505567> ${üye}\n**Eski Oda:** \`${oldState.channel.name}\`\n**Yeni Oda:** ${newState.channel}`)
                .setColor('#3498db').setTimestamp();
            logChan.send({ embeds: [embed] }).catch(() => {});
        }
    });
    ...
    client.on('guildBanAdd', async (ban) => {
        ...
        const embed = new EmbedBuilder()
            .setTitle('<:yasaklandi:1505146022588842095> Sunucudan Biri Yasaklandı! (Ban)')
            .setDescription(`<:uzaybot_kullanicilar:1505146190973505567> **Yasaklanan:** **${ban.user.tag}** \`(${ban.user.id})\`\n📄 **Sebep:** *${ban.reason || 'Belirtilmemiş.'}*`)
    ...
    client.on('guildBanRemove', async (ban) => {
        ...
        const embed = new EmbedBuilder()
            .setTitle('<:riva_kilit:1505203119427162192> Sunucudan Ban Kaldırıldı!')
