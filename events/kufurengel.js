...
module.exports = (client) => {
    ...
    // Genişletilebilir Küfür Listesi kanka
    ...
    client.on('messageCreate', async (message) => {
        ...
        // ⚠️ ADMİN KONTROLÜNÜ SİLDİK! Artık Yöneticiler de bu filtreye takılacak.
        ...
        if (tetiklendi) {
            try {
                // Mesajı kim olursa olsun saniyesinde siliyoruz
                await message.delete();

                // Kullanıcıya şık bir uyarı mesajı gönderip 5 saniye sonra uyarısını siliyoruz
                const uyariMsg = await message.channel.send(`<a:uyari:1505166167189487757> ${message.author}, bu sunucuda **${sebep}** kullanımı Yöneticiler dahil HERKES için yasaktır kanka!`);
